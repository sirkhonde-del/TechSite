...
function initializeEventListeners() {
    ...
    // Reset form styling when inputs change
    ...
    quantity.addEventListener('change', updateFormValidation);
}
...
function calculateQuote() {
    ...
    const pricePerAsset = (baseProduction + usageRightsAdd + techFee + rushFee) * turnaroundMultiplier;

    // Total with quantity
    const totalPrice = pricePerAsset * quantity;

    // Display results
    displayQuote({
        baseProduction: baseProduction.toFixed(2),
        platformMultiplier: platformMultiplier.toFixed(2),
        usageRightsAdd: usageRightsAdd.toFixed(2),
        rushFee: rushFee.toFixed(2),
        techFee: techFee.toFixed(2),
        pricePerAsset: pricePerAsset.toFixed(2),
...
function displayQuote(data) {
    ...
    document.getElementById('multiplier').textContent = `${data.platformMultiplier}x`;
...
function downloadQuote() {
    ...
    const turnaround = document.getElementById('turnaround').value;
    const quantity = parseInt(document.getElementById('quantity').value) || 1;
    const baseFee = document.getElementById('baseFee').textContent;
...
PROJECT DETAILS
=====================================
Project Type: ${formatLabel(projectScope)}
Platform: ${formatLabel(platform)}
Usage Rights: ${formatLabel(usageRights)}
Turnaround: ${formatLabel(turnaround)}
Quantity: ${quantity} asset(s)

PRICING BREAKDOWN
=====================================
Base Production Fee: ${baseFee}
Complexity Multiplier: ${document.getElementById('multiplier').textContent}
Usage Rights Add-on: ${document.getElementById('usageAdd').textContent}
Rush/Expedited Fee: ${document.getElementById('rushFee').textContent}
Tech/Finance Premium: ${document.getElementById('techFee').textContent}

TOTAL PRICE
=====================================
${totalPrice} per asset
Total for ${quantity} asset(s): ${totalPrice}
...
function selectPackage(packageId, price) {
    ...
    alert(`You selected the ${pkg.name} package ($${price.toLocaleString('en-US')}).\n\nPlease contact us at info@mwalimukonde.tech to proceed with booking.`);
    showNotification(`${pkg.name} package selected!`, 'success');
}
